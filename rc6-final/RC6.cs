...
namespace rc6_final
{
    ...
    public class RC6 : ICoderBlok
    {
        private const int Ronds = 20; // колличество раундов
        private static uint[] RoundKey = new uint[2 * Ronds + 4];  // ключ раунда
        ...
        private static void GenerateKey(int Long, byte[] keyCheck)
        {
            ...
            switch (Long)
            {
                case 128:// длина ключа
                    words_count = 4; // кол-во слов в ключе
                    break;
                case 192:// длина ключа
                    words_count = 6; // кол-во слов в ключе
                    break;
                case 256: // длина ключа
                    words_count = 8; // кол-во слов в ключе
                    break;
            }
            uint[] L = new uint[words_count];
            ...
            RoundKey[0] = P32;
            for (i = 1; i < 2 * Ronds + 4; i++)
                RoundKey[i] = RoundKey[i - 1] + Q32; // прибавление к раундовому ключу константу
            uint A, B; // регистры
            A = B = 0;
            i = j = 0;
            int Max = 3 * Math.Max(words_count, 2 * Ronds + 4);  // максимум из раундов или количества слов в ключе
            for (int s = 1; s <= Max; s++)
            {
        ...
        public byte[] EncodeBlok(byte[] byteText)
        {
            uint A, B, C, D;
            int text_length = byteText.Length;
            while (text_length % 16 != 0)
                text_length++;
            ...
            for (i = 0; i < text.Length; i = i + 16)
            {
                ...
                A = A + RoundKey[2 * Ronds + 2];
                C = C + RoundKey[2 * Ronds + 3];
        ...
        public byte[] DecodeBlok(byte[] cipherText)
        {
            uint A, B, C, D;
            int i;
